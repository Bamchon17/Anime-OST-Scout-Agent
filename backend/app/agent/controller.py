...
class AgentController:
    ...
    async def run(self, query: str, state: AgentState) -> AgentState:
        ...
        while not state.is_done():
            ...
            planning_duration_ms = int((time.perf_counter() - planning_started) * 1000)
            ...
            observation.observe_duration_ms = int((time.perf_counter() - observe_started) * 1000)
            observation.planning_duration_ms = planning_duration_ms
            observation.action_duration_ms = action_duration_ms
            observation.duration_ms = int((time.perf_counter() - loop_started) * 1000)
        ...
        if state.last_observation and state.last_observation.retrieved_data:
            ...
            state.final_response_duration_ms = int((time.perf_counter() - synthesis_started) * 1000)
        else:
