...
class Planner:
    ...
    def _apply_rule_based(self, query: str, context: str) -> Optional[ActionPlan]:
        ...
        # --- [Rule-based: Music] ---
        music_keywords = ["เพลง", "music", "ost", "soundtrack"]

        emotion_keywords = [
            "เศร้า", "ร้องไห้", "เหงา", "อบอุ่น",
            "ฝน", "คิดถึง", "ชิล", "ผ่อนคลาย",
            "dark", "sad", "emotional", "relax",
            "rain", "lonely", "healing", "nostalgic",
        ]

        if any(k in q_lower for k in music_keywords):

            # ถ้าเป็น emotional / vibe query
            # ให้ใช้ semantic search แทน music lookup
            if any(e in q_lower for e in emotion_keywords):
                return ActionPlan(
                    intent=Intent.RECOMMEND,
                    selected_tool=ToolName.RECOMMEND_TOOL,
                    retrieval_strategy=RetrievalStrategy.SEMANTIC,
                    rewritten_query=query,
                    reasoning="Emotion-aware routing: ใช้ semantic search สำหรับ music mood query",
                    confidence=0.95
                )

            # music lookup สำหรับ query ตรงๆ
            return ActionPlan(
                intent=Intent.RECOMMEND,
                selected_tool=ToolName.MUSIC_TOOL,
                retrieval_strategy=RetrievalStrategy.SEMANTIC,
                rewritten_query=query,
                reasoning="Rule-based: ตรวจพบคีย์เวิร์ดเพลงแบบเฉพาะเจาะจง",
                confidence=1.0
            )
